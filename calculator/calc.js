$(function(){
	...
	$('#autoCost_val').focusin(function(){
		...
		$(".err_autoCost_val").text("Введите стоимость автомобиля от " + fn_replace(min_autoCost) + " до " + fn_replace(max_autoCost) + " руб.").slideDown();
		$("#autoCost_val").on("change keyup", function(){
			...
			if(parseInt(val_autoCost) >= parseInt(min_autoCost)){
				if(parseInt(val_autoCost) <= parseInt(max_autoCost)){
					$(".err_autoCost_val").text("Введите стоимость автомобиля от " + fn_replace(min_autoCost) + " до " + fn_replace(max_autoCost) + " руб.");
				} else {
					$(".err_autoCost_val").text("Максимальная стоимость автомобиля " + fn_replace(max_autoCost) + " руб.");
				}
			} else {
				$(".err_autoCost_val").text("Минимальная стоимость автомобиля " + fn_replace(min_autoCost) + " руб.");
	...
	$('#firstPayment_val').focusin(function(){
		...
		$(".err_firstPayment_val").text("Введите первоначальный взнос от " + fn_replace(min_firstPayment) + " до " + fn_replace(max_firstPayment) + " руб.").slideDown();
		$("#firstPayment_val").on("change keyup", function(){
			...
			if(parseInt(val_firstPayment) >= parseInt(min_firstPayment)){
				if(parseInt(val_firstPayment) <= parseInt(max_firstPayment)){
					$(".err_firstPayment_val").text("Введите первоначальный взнос от " + fn_replace(min_firstPayment) + " до " + fn_replace(max_firstPayment) + " руб.");
				} else {
					$(".err_firstPayment_val").text("Максимальная первоначальный взнос " + fn_replace(max_firstPayment) + " руб.");
				}
			} else {
				$(".err_firstPayment_val").text("Минимальный первоначальный взнос " + fn_replace(min_firstPayment) + " руб.");
	...
	$('#duration_val').focusin(function(){
		...
		$(".err_duration_val").text("Введите срок кредита от " + min_duration + " до " + max_duration + " мес.").slideDown();
		$("#duration_val").on("change keyup", function(){
			...
			if(parseInt(val_duration) >= parseInt(min_duration)){
				if(parseInt(val_duration) <= parseInt(max_duration)){
					$(".err_duration_val").text("Введите срок кредита от " + min_duration + " до " + max_duration + " мес.");
				} else {
					$(".err_duration_val").text("Максимальная срок кредита " + max_duration + " мес.");
				}
			} else {
				$(".err_duration_val").text("Минимальная срок кредита " + min_duration + " мес.");
...
function initCalc(){
	if($('#autoCost').length > 0 && $('#autoCost_val').length > 0){
		...
		range_auto.slider({
			...
			change: function (event, ui) {
				ui.value = (ui.value+'').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ');
	...
	if($('#firstPayment').length > 0 && $('#firstPayment_val').length > 0){
		var firstPayment = parseInt($("#firstPayment_val").val().replace(/\D+/g,""));
...
function calcKredit(){
	...
	var kredit = parseInt($("#autoCost_val").val().replace(/\D+/g,"")) - parseInt($("#firstPayment_val").val().replace(/\D+/g,""));
  var cost = parseInt($('#autoCost_val').val().replace(/\D+/g,""));
  var firstPay = parseInt($('#firstPayment_val').val().replace(/\D+/g,""));
	if($('#izhcom_card').is(':checked')){
	...
	$('#payments').html((platezh+'').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '));
	$('#persent').html((kredit+'').replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 '));
}
